...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
  const { themeMode, toggleTheme, logout, user, activityLogs } = useApp();
  ...
  // Compute pending approvals (PENDING or UNDER_REVIEW)
  const pendingApprovalsCount = activityLogs?.filter(log => log.status === 'PENDING' || log.status === 'UNDER_REVIEW').length || 0;
          ...
          return (
            <ListItem key={item.text} disablePadding sx={{ mb: 1 }}>
              ...
              >
                ...
                >
                  {item.path === '/work-monitoring' && pendingApprovalsCount > 0 ? (
                    <Badge badgeContent={pendingApprovalsCount} color="error" sx={{ '& .MuiBadge-badge': { fontWeight: 800, fontSize: '0.65rem' } }}>
                      {item.icon}
                    </Badge>
                  ) : (
                    item.icon
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5 }}>
            ...
            >
              <Box sx={{ px: 2.5, py: 1.8 }}>
                ...
                <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600 }}>{user?.userId || 'admin@workforce-management.com'}</Typography>
              </Box>
              <Divider />
              <MenuItem onClick={() => { handleCloseUserMenu(); navigate('/settings'); }} sx={{ py: 1.2, px: 2.5 }}>My Profile</MenuItem>
              <MenuItem onClick={() => { handleCloseUserMenu(); navigate('/settings'); }} sx={{ py: 1.2, px: 2.5 }}>Security Settings</MenuItem>
              <Divider />
