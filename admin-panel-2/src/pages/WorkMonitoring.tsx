...
export const WorkMonitoring: React.FC = () => {
  ...
  const handleApproveWork = async (logId?: string) => {
    const idToApprove = typeof logId === 'string' ? logId : selectedLog?.id;
    if (!idToApprove) return;
    const success = await updateActivityLog(idToApprove, {
      status: 'APPROVED',
    });
    if (success) {
      if (selectedLog && selectedLog.id === idToApprove) {
  ...
  const handleRejectWork = async (logId?: string) => {
    const idToReject = typeof logId === 'string' ? logId : selectedLog?.id;
    if (!idToReject) return;
    const success = await updateActivityLog(idToReject, {
      status: 'REJECTED',
    });
    if (success) {
      if (selectedLog && selectedLog.id === idToReject) {
  ...
  const getLogStatus = (log: ActivityLog): string => {
  ...
  const getStatusChip = (status: string) => {
    switch (status) {
      case 'COMPLETED':
        return (
          <Chip
            icon={<CompleteIcon sx={{ fontSize: '14px !important', color: '#EAB308 !important' }} />}
            label="Completed"
            size="small"
            sx={{
              fontWeight: 800,
      ...
      case 'PENDING':
        return (
          <Chip
            icon={<PendingIcon sx={{ fontSize: '14px !important', color: '#9CA3AF !important' }} />}
            label="Pending"
            size="small"
            sx={{
              fontWeight: 800,
      ...
      case 'APPROVED':
        return (
          <Chip
            icon={<CompleteIcon sx={{ fontSize: '14px !important', color: '#059669 !important' }} />}
            label="Approved"
            size="small"
            sx={{
              fontWeight: 800,
      ...
      case 'REWORK_ASSIGNED':
        return (
          <Chip
            icon={<RunningIcon sx={{ fontSize: '14px !important', color: '#EF4444 !important' }} />}
            label="Rework"
            size="small"
            sx={{
              fontWeight: 800,
      ...
      case 'UNDER_REVIEW':
        return (
          <Chip
            icon={<ReviewIcon sx={{ fontSize: '14px !important', color: '#8B5CF6 !important' }} />}
            label="Under Review"
            size="small"
            sx={{
              fontWeight: 800,
          ...
          <Grid size={{ xs: 12, md: 5.5 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
              {[
                ...
                { value: 'PENDING', label: 'Pending', icon: <PendingIcon sx={{ fontSize: 15 }} />, color: '#9CA3AF', bg: 'rgba(156,163,175,0.08)' },
                { value: 'IN_PROGRESS', label: 'In Progress', icon: <RunningIcon sx={{ fontSize: 15 }} />, color: '#3B82F6', bg: 'rgba(59,130,246,0.08)' },
                { value: 'COMPLETED', label: 'Completed', icon: <CompleteIcon sx={{ fontSize: 15 }} />, color: '#10B981', bg: 'rgba(16,185,129,0.08)' },
                { value: 'REWORK_ASSIGNED', label: 'Rework', icon: <RunningIcon sx={{ fontSize: 15 }} />, color: '#F59E0B', bg: 'rgba(245,158,11,0.08)' },
                { value: 'UNDER_REVIEW', label: 'Under Review', icon: <ReviewIcon sx={{ fontSize: 15 }} />, color: '#8B5CF6', bg: 'rgba(139,92,246,0.08)' },
                { value: 'APPROVED', label: 'Approved', icon: <CompleteIcon sx={{ fontSize: 15 }} />, color: '#059669', bg: 'rgba(5,150,105,0.08)' },
                        ...
                        {isAdmin && (
                          <>
                            <Tooltip title="Review & Remarks">
                              <IconButton size="small" color="warning" onClick={(e) => { e.stopPropagation(); handleOpenDetails(log, 3); }}>
                                <ReviewIcon sx={{ fontSize: 18 }} />
                              </IconButton>
                            </Tooltip>
                            {status !== 'APPROVED' && (
                              <Tooltip title="Quick Approve">
                                <IconButton size="small" color="success" onClick={(e) => { e.stopPropagation(); handleApproveWork(log.id); }}>
                            ...
                            {status !== 'REJECTED' && (
                              <Tooltip title="Quick Reject">
                                <IconButton size="small" color="error" onClick={(e) => { e.stopPropagation(); handleRejectWork(log.id); }}>
